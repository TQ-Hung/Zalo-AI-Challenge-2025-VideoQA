# src/inference.py
# BẢN DÀNH CHO TRƯỜNG HỢP ĐÃ EXTRACT TRƯỚC TEST FEATURES
import os
import json
import torch
import numpy as np
...
FEATURE_DIR = "/kaggle/working/features_test"   # <--- thư mục bạn đã extract
APP_DIR = f"{FEATURE_DIR}/appearance"
MOT_DIR = f"{FEATURE_DIR}/motion"
OCR_DIR = f"{FEATURE_DIR}/ocr"                  # optional
...
class PublicTestDataset(Dataset):
    def __init__(self):
        ...
        for item in data:
            ...
            if not os.path.exists(app_path) or not os.path.exists(mot_path):
                continue
    ...
    def __getitem__(self, idx):
        ...
        # ---- Load appearance / motion ----
        appearance = np.load(item["app_path"])        # (1,2048)
        motion = np.load(item["mot_path"])            # (1,2048)

        # ---- OCR ----
        ocr_text = ""
        ocr_file = os.path.join(OCR_DIR, f"{item['video_id']}.txt")
    ...
    for seed in ENSEMBLE_SEEDS:
        ...
        with torch.no_grad():
            for batch in tqdm(test_loader, desc=f"Seed {seed}"):
                ...
                encoded = tokenizer(
                    batch["questions"], padding=True, truncation=True,

...
# DINOv2 + support_frames + 2 GPU DataParallel
import os
import torch
import cv2
import numpy as np
...
OUTPUT_DIR = "features_v2/appearance"
...
for item in train_data:
    ...
    video_to_supports[basename] = [float(ts) for ts in supports if ts.strip()]
...
if torch.cuda.device_count() > 1:
    print(f"Using {torch.cuda.device_count()} GPUs for DINOv2!")
...
def extract_single(video_path):
    ...
    feats = outputs.last_hidden_state.mean(dim=1)  # (16, 768)
...
if __name__ == "__main__":
    ...
    skipped = 0
    for vp in tqdm(videos, desc="Appearance (2 GPU)"):
    ...
    print(f"DONE appearance! Saved {len(videos)-skipped}, skipped {skipped}")

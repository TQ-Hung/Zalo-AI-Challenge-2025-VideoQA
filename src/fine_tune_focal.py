# /kaggle/working/Zalo-AI-Challenge-2025-VideoQA/src/fine_tune_focal.py
import os
import torch
import random
import numpy as np
...
EPOCHS = 15
WEIGHT_DECAY = 0.01
OUTPUT_DIR = "/kaggle/working/checkpoints_fine_tune_focal"
VALID_SPLIT = 0.1
SEED = 42
...
def evaluate_focal(model, loader, device, criterion):
    model.eval()
    total_loss = 0.0
    total = 0
    correct = 0
...
def fine_tune_focal():
    ...
    for epoch in range(EPOCHS):
        ...
        pbar = tqdm(train_loader, desc=f"Epoch {epoch+1}/{EPOCHS} [Focal Fine-tuning]")
        ...
        # Validation
        val_loss, val_acc = evaluate_focal(model, val_loader, device, criterion)
        ...
        if val_acc > best_val_acc:
            ...
            torch.save({
                ...
                "epoch": epoch,
                "val_acc": val_acc
            }, os.path.join(OUTPUT_DIR, "best_focal.pt"))
            print("✅ Saved best focal fine-tuned checkpoint.")
    
    print(f"🎉 Focal fine-tuning done. Best val: {best_val_acc:.4f}")
